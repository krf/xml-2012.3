...
function addPointsOfInterestToMap(id) {
	...
	if (urlExists) {
		...
		$(document).ready(
				function() {
					var poiDataArray = $("#poilist").children();
					for ( var poiIndex = 0; poiIndex < poiDataArray.length; poiIndex++) {
						currentPoi = poiDataArray[poiIndex];
						currentPoiMeta = $("span > meta[itemprop]", currentPoi);
						marker = new google.maps.Marker({
							animation : google.maps.Animation.DROP,
							map : map,
							position : new google.maps.LatLng(currentPoiMeta[0].attributes[1].value,
									currentPoiMeta[1].attributes[1].value)
						});
						infoWindow = new google.maps.InfoWindow({
							content : $("div", currentPoi).html()
						});
